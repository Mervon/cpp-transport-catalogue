...
void ParseLineWithStop(ParsedLines& requests, string& str) {
...
void ParseLineWithBus(ParsedLines& requests, string& str) {
    str = str.substr(str.find_first_of(' '));
    auto it_1 = str.find_first_not_of(' ');
    auto it_2 = str.find_first_of(':');
...
ParsedLines ParseLines(vector<string>& strings) {
    ...
    for (auto& str : strings) {
        if (str[0] == 'S') {
            ParseLineWithStop(requests, str);
        } else if (str[0] == 'B') {
            ParseLineWithBus(requests, str);
        }
    }
    return requests;
}
